...
import { hasClass, addClass, removeClass } from "@/constants/util/dom-util";
import { isServer } from "../util";
import { TableColumnCtx, Table } from "../table.type";
import { TableHeaderProps } from "./table-header";

function useEvent(props: TableHeaderProps, emit: any) {
  ...
  const handleSortClick = (event: Event, column: TableColumnCtx, givenOrder: string | boolean) => {
    event.stopPropagation();
    ...
    if (!order) {
      sortOrder = column.order = null as any;
    } else {
