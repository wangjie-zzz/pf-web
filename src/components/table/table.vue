  ...
  >
    <div ref="hiddenColumns" class="hidden-columns">
      <template v-for="col in config.columns" :key="col.prop">
        <!--:width="col.width" TODO 宽度问题 -->
        <el-table-column v-if="!col.type" :prop="col.prop" :label="col.label" :fixed="col.fixed" :type="col.type">
          <template #default="scope">
            {{ convertDict(col.dict, scope.row[col.prop]) }}
          </template>
        </el-table-column>
        <el-table-column v-else :prop="col.prop" :label="col.label" :width="col.width" :fixed="col.fixed" :type="col.type"> </el-table-column>
      </template>
      <slot></slot>
    </div>
    <!--showHeader: table-header :store-->
    ...
    <!--showHeader: table-body :store :context-->
    <div ref="bodyWrapper" :style="[bodyHeight]" class="el-table__body-wrapper">
      ...
      />
      <div v-if="!data || data.length === 0" ref="emptyBlock" :style="emptyBlockStyle" class="el-table__empty-block">
    ...
    <!--showSummary: table-footer :store -->
    ...
    <!--store.states.fixedColumns fixedWrapper固定列-->
    <div
    ...
    <!--store.states.rightFixedColumns rightFixedWrapper-->
    <div
    ...
    >
      ...
      <div ref="rightFixedBodyWrapper" :style="[{ top: layout.headerHeight.value + 'px' }, fixedBodyHeight]" class="el-table__fixed-body-wrapper">
        <table-body
          :highlight="highlightCurrentRow"
          :row-class-name="rowClassName"
          :tooltip-effect="tooltipEffect"
    ...
    <!--store.states.rightFixedColumns rightFixedPatch-->
    <div
      v-if="store.states.rightFixedColumns.value.length > 0"
      ref="rightFixedPatch"
      :style="{
...
<script lang="ts">
/* eslint-disable */
import {defineComponent, getCurrentInstance, computed, toRef, toRefs, reactive} from "vue";
import { createStore } from "./store/helper";
import { Mousewheel } from "@/components/table/directives/mousewheel";
...
export default defineComponent({
  ...
  props: {
    ...
    lazy: Boolean,
    load: Function,
    config: {type: TableModel, required: true, default() { return {} }},
    recordTotal: {type: Number, default() {return 0}}
  },
  emits: ["select", "select-all", "selection-change", "cell-mouse-enter", "cell-mouse-leave", "cell-click", "cell-dblclick", "row-click", "row-contextmenu", "row-dblclick", "header-click", "header-contextmenu", "sort-change", "filter-change", "current-change", "header-dragend", "expand-change", "page-change"],
  setup(props: any, { emit }) {
    ...
    table.store = store;
    const layout: any = new TableLayout({
      store: table.store,
      table,
      fit: props.fit,
     ...
     * open functions
     */
    const { setCurrentRow, toggleRowSelection, clearSelection, clearFilter, toggleAllSelection, toggleRowExpansion, clearSort, sort } = useUtils(store);
    const { isHidden, renderExpanded, setDragVisible, isGroup, handleMouseLeave, handleHeaderFooterMousewheel, tableSize, bodyHeight, emptyBlockStyle, handleFixedMousewheel, fixedHeight, fixedBodyHeight, resizeProxyVisible, bodyWidth, resizeState, doLayout } = useStyle(props, layout, store, table);
    ...
    let pageSize = 10;
    let pageNum = 1;
    const recordTotal = toRef(props,"recordTotal");
    const pageSizeChange = (val: number) => {
      if(typeof val === "number" && val !== pageSize) {
        pageSize = val;
        if(((pageNum - 1) * pageSize) < recordTotal.value) emit("page-change", {pageSize, pageNum});
      }
    };
    const pageChange = (val: number) => {
      if(typeof val === "number" && val !== pageNum) {
        pageNum = val;
        if(((pageNum - 1) * pageSize) < recordTotal.value) emit("page-change", {pageSize, pageNum});
      }
    };
